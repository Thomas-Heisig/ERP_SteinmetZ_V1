...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Add job to queue
    const job = await aiBatchQueue.add(
      "annotate",
      {
...
createBullBoard({
  queues: [new BullMQAdapter(aiBatchQueue), new BullMQAdapter(exportQueue)],
  serverAdapter,
});
...
```typescript
{
  ...
  repeat: {
    cron: '0 9 * * 1', // Every Monday at 9:00 AM (server timezone)
    // Note: Configure TZ environment variable for consistent timezone in production
  },
  priority: 2,
}
```

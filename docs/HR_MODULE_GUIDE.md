...
interface Absence {
  id: string;
  employee_id: string;
  absence_type: 'vacation' | 'sick_leave' | 'parental_leave' | 'unpaid_leave' | 'training';
...
router.get('/absences', asyncHandler(async (req, res) => {
  ...
  const absences = await hrService.getAbsences({ employee_id, status, start_date, end_date });
...
export const absenceSchema = z.object({
  ...
  absence_type: z.enum(['vacation', 'sick_leave', 'parental_leave', 'unpaid_leave', 'training']),
...
export class HRService {
  ...
  async updateEmployee(id: string, updates: Partial<Employee>): Promise<Employee> {
    ...
    await dbService.update('employees', { id }, {
  ...
  private mapEmployeeToRow(employee: Employee): any {
    return {
      ...
      employment: JSON.stringify(employee.employment),
      compensation: employee.compensation ? JSON.stringify(employee.compensation) : null,
...
async function scheduleDocumentDeletion() {
  ...
  for (const doc of documents) {
    ...
    await db.update('employee_documents', { id: doc.id }, {
...
// Datenauskunft
router.get('/employees/:id/data-export',
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="employee-data-${id}.json"`);
    res.json(data);
  })
);
```

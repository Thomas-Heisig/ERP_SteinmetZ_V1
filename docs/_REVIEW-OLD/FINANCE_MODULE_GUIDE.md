...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="rechnung-${id}.pdf"`,
    );
    res.send(pdfBuffer);
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="xrechnung-${id}.xml"`,
    );
    res.send(xmlBuffer);
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="datev-export.csv"`,
    );
    res.send(csvBuffer);
  }),
);
...
export class FinanceService {
  ...
  async createInvoice(
    ...
    // Beträge berechnen
    const { subtotal, tax_total, total } = this.calculateInvoiceTotals(
      data.line_items!,
    );
  ...
  async recordPayment(
    ...
    await dbService.insert("payments", {
      ...
      bank_account: payment.bank_account
        ? JSON.stringify(payment.bank_account)
        : null,

...
export interface ConversationState {
  ...
  intent?: 'query' | 'create' | 'update' | 'delete' | 'calculate' | 'diagnose' | 'informational';
...
export const useAI = () => {
  ...
  const getSession = useCallback(async (sessionId: string): Promise<ChatSession> => {
    ...
    throw new Error('Direct session access not available. Use listSessions instead.');
  }, [apiCall]);
  ...
  const resetContext = useCallback(async (): Promise<void> => {
    ...
    console.warn('Context reset not directly available. Create a new session instead.');
  }, [apiCall]);
  ...
  const updateSettings = useCallback(async (settings: any): Promise<any> => {
    return apiCall('/settings', {
      method: 'PUT',
  ...
  const updateSetting = useCallback(async (key: string, value: any): Promise<any> => {
  ...
  const queryKnowledge = useCallback(async (query: string, limit: number = 5): Promise<any> => {

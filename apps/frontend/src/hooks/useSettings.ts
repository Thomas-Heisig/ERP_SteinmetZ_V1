...
export interface UseSettingsReturn {
  ...
  updateMultiple: (updates: Record<string, unknown>, reason?: string) => Promise<void>;
  ...
  const updateMultiple = useCallback(
    async (updates: Record<string, unknown>, reason?: string) => {
      try {
        ...
        // Update local state
        setSettings((prev) =>
          prev.map((s) => (updates[s.key] !== undefined ? { ...s, value: updates[s.key] } : s))
        );
...
export function useSetting<T = unknown>(key: string, defaultValue?: T): {
  ...
  const { getSetting, getSettingValue, updateSetting, loading, error } = useSettings();

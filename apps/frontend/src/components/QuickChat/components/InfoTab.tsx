...
const getStatusText = (status: string | undefined | null): string => {
  ...
  switch (status) {
    case 'healthy': return 'System ist gesund';
    case 'degraded': return 'System eingeschränkt';
    case 'unhealthy': return 'System hat Probleme';
...
const formatBoolean = (value: boolean | undefined | null): { icon: string; text: string } => {
...
const getProviderInfo = (provider: string | undefined | null): { name: string; description: string } => {
  if (!provider) return { name: 'Unbekannt', description: 'Provider nicht erkannt' };
    ...
    <section className="info-tab">
      ...
      <div className="info-grid">
        ...
        <div className="info-card status-card">
          ...
          <div className="status-overview">
            <div className={`system-status ${getStatusColor(systemStatusValue)}`}>
              <span className="status-icon">{getStatusIcon(systemStatusValue)}</span>
          ...
          <div className="info-item">
            ...
            <span className={fallbackEnabled ? 'feature-enabled' : 'feature-disabled'}>
          ...
          <div className="providers-grid">
                ...
                <div key={provider} className="provider-status">
                  <div className="provider-header">
                    ...
                    <span className="status-indicator">
                      {isActive ? '🟢 Aktiv' : 
                       isFallback && fallbackEnabled ? '🟡 Bereit' : '⚪ Standby'}
                    </span>
                  </div>
            ...
            {activeProvider && !commonProviders.includes(activeProvider) && (
              <div className="provider-status highlighted">
                <div className="provider-header">
                  <span className="provider-icon">{getProviderIcon(activeProvider)}</span>
                  <span className="provider-name">{activeProviderInfo.name}</span>
        ...
        <div className="info-card features-card">
          ...
          <div className="features-grid">
            <div className={`feature-item ${modelCount > 0 ? 'enabled' : 'disabled'}`}>
            ...
            <div className={`feature-item ${toolCount > 0 ? 'enabled' : 'disabled'}`}>
            ...
            <div className={`feature-item ${workflowCount > 0 ? 'enabled' : 'disabled'}`}>
              <span className="feature-icon">🔄</span>
              <div className="feature-info">
            ...
            <div className={`feature-item ${fallbackEnabled ? 'enabled' : 'disabled'}`}>
              <span className="feature-icon">🔄</span>
              <div className="feature-info">
      ...
      <div className="info-card full-width health-card">
        ...
        <div className="health-grid">
          <div className={`health-item ${systemStatusValue === 'healthy' ? 'healthy' : 'degraded'}`}>
            <span className="health-icon">{getStatusIcon(systemStatusValue)}</span>
            <div className="health-info">
              <strong>Gesamtsystem</strong>
              <span>{getStatusText(systemStatusValue)}</span>
            </div>
          </div>
          <div className={`health-item ${modelCount > 0 ? 'healthy' : 'degraded'}`}>
          ...
          <div className={`health-item ${toolCount > 0 ? 'healthy' : 'degraded'}`}>
          ...
          <div className={`health-item ${fallbackEnabled ? 'healthy' : 'warning'}`}>

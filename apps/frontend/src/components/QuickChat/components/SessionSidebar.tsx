...
export const SessionSidebar: React.FC<SessionSidebarProps> = ({
  ...
  const getSessionTitle = (session: ChatSession) => {
    ...
    const firstContent =
      firstUserMessage?.content || safeMessages[0]?.content || "";
  ...
  const getSessionIcon = (session: ChatSession) => {
    ...
    if (firstMessage.includes("frage") || firstMessage.includes("?"))
      return "❓";
    if (firstMessage.includes("hilfe") || firstMessage.includes("support"))
      return "🆘";
    if (firstMessage.includes("analyse") || firstMessage.includes("daten"))
      return "📊";
    if (firstMessage.includes("code") || firstMessage.includes("programm"))
      return "💻";
  ...
  const filteredSessions = safeSessions.filter((session) => {
    ...
    const tagsMatch =
      Array.isArray((session as any).tags) &&
      (session as any).tags.some(
        (tag: string) =>
          ...
          <div className="empty-sessions">
            {searchTerm ? (
              ...
              <>
                ...
                <button className="empty-action-btn" onClick={onSessionCreate}>
...
const SessionItem: React.FC<SessionItemProps> = ({
  ...
  const availableModels = safeModels.filter(
    (m) => m && typeof m.name === "string" && m.name === session.model,
  );
  ...
  const tags = Array.isArray((session as any).tags)
    ? (session as any).tags
    : [];
  ...
  return (
    ...
    >
      <div className="session-main" onClick={() => onSelect(session.id)}>
        <div className="session-icon">{sessionIcon}</div>

        <div className="session-info">
          <div className="session-header">
            <div className="session-title truncate">{sessionTitle}</div>
      ...
      {showDeleteConfirm && (
        <div className="delete-confirmation">
          <div className="confirmation-text">Unterhaltung löschen?</div>

...
export const MessageList: React.FC<MessageListProps> = ({
  ...
  const getModelName = () => {
    ...
    const model = safeModels.find(
      (m) => m.name === currentModel || m.model === currentModel,
    );
  ...
  const renderMetadata = (metadata: ChatMessage["metadata"]) => {
    ...
    if (
      Array.isArray(metadata.context_references) &&
      metadata.context_references.length > 0
    ) {
      metadataItems.push(
            ...
            <div key={index} className="tool-call">
              ...
              {toolCall.result !== undefined && (
                <div className="tool-result">
                  ...
                  <pre
                    className={toolCall.success === false ? "tool-error" : ""}
                  >
  ...
  const renderContextInfo = () => {
    ...
    if (typeof conversationContext.intent === "string") {
      contextItems.push(
        <span key="intent" className="context-item">
          <strong>Intent:</strong>
          <span
            className={`intent-${conversationContext.intent.toLowerCase()}`}
          >
      ...
      if (typeof conversationContext.stats.tokensUsed === "number") {
        contextItems.push(
          <span key="tokens" className="context-item">
            ...
            {conversationContext.stats.tokensUsed.toLocaleString()}
          </span>,
        );
      }
      ...
      if (typeof conversationContext.stats.topicSwitches === "number") {
        contextItems.push(
          <span key="switches" className="context-item">
            <strong>Themenwechsel:</strong>{" "}
            {conversationContext.stats.topicSwitches}
          </span>,
        );
      }
    }
    ...
    if (
      ...
      typeof conversationContext.preferences === "object" &&
      Object.keys(conversationContext.preferences).length > 0
    ) {
      contextItems.push(
        <span key="preferences" className="context-item">
          <strong>Präferenzen:</strong>{" "}
          {Object.keys(conversationContext.preferences).length} gesetzt
        </span>,
      );
    }
    ...
    return (
      <div className="context-info-banner">
        ...
        <div className="context-details">{contextItems}</div>
      </div>
    );
  };
  ...
  const renderWelcomeMessage = () => {
    const safeModels = Array.isArray(models) ? models : [];
    const currentModelData = safeModels.find(
      (m) => m.name === currentModel || m.model === currentModel,
    );
    ...
    return (
      <div className="welcome-message">
        ...
        <p className="welcome-description">
          Ich bin Ihr KI-Assistent und helfe Ihnen bei verschiedenen Aufgaben im
          ERP-System. Nutzen Sie die erweiterten Funktionen für
          Textverarbeitung, Datenanalyse, Geschäftsprozesse und Automatisierung.
        </p>
    ...
    if (!msg || typeof msg !== "object") {
      ...
      return (
        <div key={`invalid-${idx}`} className="message error-message">
          ...
          <div className="message-content">
            <div className="message-text">Ungültige Nachricht empfangen</div>
          </div>
        </div>
      );
    }
      ...
      >
        ...
        <div className="message-content">
          <div className="message-header">
            <span className="message-sender">
              ...
              {msg.metadata?.provider && safeRole === "assistant" && (
                <span className="provider-badge">
                  {getProviderIcon(msg.metadata.provider)}{" "}
          ...
          {Array.isArray(msg.metadata?.tool_calls) &&
            renderToolCalls(msg.metadata.tool_calls)}

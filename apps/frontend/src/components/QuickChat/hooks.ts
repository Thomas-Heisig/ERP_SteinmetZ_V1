...
import { ChatSession, AIModel, Settings, ToolMetadata, ConversationState, ChatMessage, AIResponse } from './types';
...
export const useQuickChat = () => {
  ...
  const [conversationContext, setConversationContext] = useState<ConversationState | null>(null);
  ...
  const apiCall = useCallback(async <T = any>(
    ...
    try {
      ...
      const message = err.message || "ERR_UNKNOWN: An unexpected error occurred";
      setError(message);
      throw err;
    } finally {
      setLoading(false);
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      if (sessionId) {
        // ✅ KORRIGIERT: Korrektes Payload gemäß API-Dokumentation
        const response = await apiCall<AIResponse>(`/chat/${sessionId}/message`, {
          method: 'POST',
        ...
        const response = await apiCall<AIResponse>(`/chat/${session.id}/message`, {
          method: 'POST',
          body: JSON.stringify({ message }),
        });
  ...
  const getProviders = useCallback(async (): Promise<Array<{
    provider: string;
    available: boolean;
  }>> => {
    try {
  ...
  const saveSettings = useCallback(async (newSettings: Settings): Promise<Settings> => {
    try {
  ...
  const updateSetting = useCallback(async (key: string, value: any): Promise<void> => {
    try {
  ...
  const loadConversationContext = useCallback(async (): Promise<ConversationState> => {
    try {
  ...
  const getConversationContext = useCallback(async (): Promise<ConversationState> => {
  ...
  const transcribeAudio = useCallback(async (audioFile: File): Promise<{
    text: string;
    meta: any;
  }> => {

...
// apps/frontend/src/features/finance/modules/ControllingList.tsx

import React, { useState, useEffect } from "react";
import { Card, Table, Button, Input } from "../../components/ui";
import styles from "./ControllingList.module.css";
...
export const ControllingList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/finance/controlling");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>📊 Controlling</h2>
...
export default ControllingList;

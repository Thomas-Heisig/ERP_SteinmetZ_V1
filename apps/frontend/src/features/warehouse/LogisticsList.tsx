...
// apps/frontend/src/features/warehouse/LogisticsList.tsx
...
export const LogisticsList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/warehouse/logistics");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>🚛 Logistik</h2>

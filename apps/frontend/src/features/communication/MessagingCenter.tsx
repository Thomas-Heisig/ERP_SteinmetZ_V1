...
// apps/frontend/src/features/communication/MessagingCenter.tsx
...
export const MessagingCenter: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/communication/messaging");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>💬 Messaging</h2>

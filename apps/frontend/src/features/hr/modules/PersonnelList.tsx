...
// apps/frontend/src/features/hr/modules/PersonnelList.tsx
...
export const PersonnelList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/hr/personnel");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>👤 Personalverwaltung</h2>

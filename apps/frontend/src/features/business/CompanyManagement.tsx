...
// apps/frontend/src/features/business/CompanyManagement.tsx
...
export const CompanyManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/business/companies");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>🏢 Unternehmensverwaltung</h2>

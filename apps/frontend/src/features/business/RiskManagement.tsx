...
// apps/frontend/src/features/business/RiskManagement.tsx
...
export const RiskManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/business/risks");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>🛡️ Risiko & Compliance</h2>

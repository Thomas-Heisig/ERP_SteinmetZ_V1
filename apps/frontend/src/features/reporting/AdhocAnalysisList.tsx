...
// apps/frontend/src/features/reporting/AdhocAnalysisList.tsx

import React, { useState, useEffect } from "react";
import { Card, Table, Button, Input } from "../../components/ui";
import styles from "./AdhocAnalysisList.module.css";
...
export const AdhocAnalysisList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/reporting/adhoc");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>🔍 Ad-hoc-Analysen</h2>
...
export default AdhocAnalysisList;

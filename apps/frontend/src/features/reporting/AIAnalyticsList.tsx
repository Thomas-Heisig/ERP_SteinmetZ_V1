...
// apps/frontend/src/features/reporting/AIAnalyticsList.tsx

import React, { useState, useEffect } from "react";
import { Card, Table, Button, Input } from "../../components/ui";
import styles from "./AIAnalyticsList.module.css";
...
export const AIAnalyticsList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/reporting/ai-analytics");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>🤖 KI-Analytics</h2>
...
export default AIAnalyticsList;

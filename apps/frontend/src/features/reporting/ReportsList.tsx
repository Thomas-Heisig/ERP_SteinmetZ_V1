...
// apps/frontend/src/features/reporting/ReportsList.tsx
...
export const ReportsList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://localhost:3000/api/reporting/reports");
        if (!response.ok) {
  ...
  return (
    <Card variant="elevated" padding="none">
      <div className={styles.header}>
        <h2 className={styles.headerTitle}>📈 Standard-Reports</h2>

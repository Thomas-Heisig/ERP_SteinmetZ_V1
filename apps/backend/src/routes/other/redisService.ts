...
class RedisService {
  ...
  private hasLoggedError = false; // Track if error was already logged
   ...
   *
   * @returns {{ connected: boolean; usingFallback: boolean }} Connection status
   *
   * @example
   * ```typescript
   * const status = redisService.getStatus();
   * console.log(`Redis connected: ${status.connected}, Using fallback: ${status.usingFallback}`);
   * ```
   */

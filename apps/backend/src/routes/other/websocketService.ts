...
import { log } from "../ai/utils/logger.js";
...
class WebSocketService {
  ...
  private setupMiddleware(): void {
    ...
    this.io.use((socket: AuthenticatedSocket, next) => {
      ...
      try {
        ...
        socket.userId = (decoded.id || decoded.userId) as string;
        socket.userRoles = (decoded.roles || []) as string[];
   ...
   * ```
   */
  toRoom(room: string, event: string, data: Record<string, unknown>): void {
    if (!this.io) {
   ...
   * ```
   */
  toUser(userId: string, event: string, data: Record<string, unknown>): void {
    if (!this.io) {

...
const JsonNodeSchema = z
  .object({
    ...
    rbac: z.record(z.string(), z.unknown()).optional(),
    flags: z.record(z.string(), z.unknown()).optional(),
    pii: z.record(z.string(), z.unknown()).optional(),
    aa: z.record(z.string(), z.unknown()).optional(),
...
function extractArea(node: JsonLikeNode): string {
...
function extractTags(node: JsonLikeNode): string[] {
...
function jsonToCatalogNode(
  ...
  const metaRecord =
    jsonNode.meta && typeof jsonNode.meta === "object"
      ? (jsonNode.meta as Record<string, unknown>)
      : undefined;
  ...
  const rawCreated = metaRecord?.created;
  const created = typeof rawCreated === "string" ? rawCreated : undefined;
  ...
  const node: CatalogNode = {
    ...
    rbac: jsonNode.rbac as NodeRBAC | undefined,
    flags: jsonNode.flags as NodeFlags | undefined,
    pii: jsonNode.pii as NodePII | undefined,
    aa: jsonNode.aa as NodeAA | undefined,
    schema: jsonNode.schema as NodeSchema | undefined,
    warnings: jsonNode.warnings as string[] | undefined,
  };

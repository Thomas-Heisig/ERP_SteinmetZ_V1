...
export function registerTools(toolRegistry: {
  ...
  calculateTool.description =
    "Führt sichere mathematische Berechnungen durch (mit √, π, %, ^ usw.)";
  calculateTool.parameters = {
    expression: 'Mathematischer Ausdruck (z. B. "√(9)+3^2")',
  };
  ...
  statisticsTool.description =
    "Berechnet statistische Kennzahlen (Mittelwert, Median, Varianz, etc.)";
  ...
  percentTool.description =
    "Berechnet Prozentwerte oder prozentuale Zu-/Abnahmen.";
  percentTool.parameters = {
    value: "Grundwert",
    percent: "Prozentwert",
    mode: "of|increase|decrease",
  };
  ...
  convertUnitTool.description =
    "Konvertiert Längen zwischen gängigen Einheiten (m, cm, inch, km, ft, yd).";
  convertUnitTool.parameters = {
    value: "Zahl",
    from: "Einheit",
    to: "Zieleinheit",
  };
  ...
  const solveEquationTool = (async ({ equation }: { equation: string }) => {
    try {
      ...
      if (!lhs || !rhs)
        throw new Error("Ungültige Gleichung. Beispiel: 2*x+4=10");

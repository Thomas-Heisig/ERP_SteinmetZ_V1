...
// apps/backend/src/routes/selfhealing/DatabaseHealthMonitor.ts
...
export class DatabaseHealthMonitor {
  ...
  private readonly logger = createLogger("db-health-monitor");
  ...
  async runHealthChecks(): Promise<HealthCheckResult> {
    if (this.isRunning) {
      ...
      return (
        this.lastCheckResult ?? {
          ...
          status: "healthy",
          checks: [],
          summary: "Health check skipped because another run is in progress",
        }
      );
    }
    ...
    try {
      ...
      const result: HealthCheckResult = {
        ...
        status: "unhealthy",
        checks,
        summary: "Health check failed due to unexpected error",
      };
      this.lastCheckResult = result;
      return result;
    } finally {
  ...
  private async checkSchemaIntegrity(): Promise<HealthCheck> {
    ...
    try {
      ...
      this.logger.error({ error }, "Schema integrity check failed");
      return {
        name: "schema_integrity",
        status: "fail",
  ...
  private async checkReferentialIntegrity(): Promise<HealthCheck> {
    ...
    try {
      ...
      this.logger.error({ error }, "Referential integrity check failed");
      return {
        name: "referential_integrity",
        status: "fail",
  ...
  private async checkDuplicates(): Promise<HealthCheck> {
    ...
    try {
      ...
      this.logger.error({ error }, "Duplicate check failed");
      return {
        name: "duplicates",
        status: "fail",
  ...
  private async checkDataValidity(): Promise<HealthCheck> {
    ...
    try {
      ...
      const invalidKind = await db.get<{ count: number }>(
        `SELECT COUNT(*) as count FROM functions_nodes WHERE kind NOT IN (${validKinds
          .map(() => "?")
          .join(",")})`,
        validKinds,
      );
      ...
      this.logger.error({ error }, "Data validity check failed");
      return {
        name: "data_validity",
        status: "fail",
  ...
  private async checkStorageHealth(): Promise<HealthCheck> {
    ...
    try {
      ...
      this.logger.error({ error }, "Storage health check failed");
      return {
        name: "storage_health",
        status: "fail",
  ...
  async findIntegrityIssues(): Promise<IntegrityIssue[]> {
    ...
    try {
      ...
      this.logger.error({ error }, "Error finding integrity issues");
    }

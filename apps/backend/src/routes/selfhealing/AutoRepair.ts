...
export class AutoRepair {
  ...
  private static readonly EDGE_DEFAULT_WEIGHT = 1;
  private static readonly EDGE_DEFAULT_RELATION = "contains";
  ...
  async startRepairSession(dryRun = false): Promise<RepairSession> {
    ...
    try {
      ...
      if (issues.length === 0) {
        ...
        logger.info({ sessionId }, "No issues found - nothing to repair");
        return session;
      }
  ...
  private async repairOrphanEdge(
    ...
    if (!parentId || !childId) {
      result.error = "Invalid recordId format for orphan edge";
      result.action = "SKIPPED";
      return;
    }
  ...
  private async rollbackResult(result: RepairResult): Promise<void> {
    const data = result.rollbackData as Record<string, unknown> | undefined;
    ...
    if (result.issue.type === "orphan_edge" && data) {
      ...
      await db.run(
        ...
        [parentId, childId, weight, relationship],
      );
    } else if (result.issue.type === "invalid_data" && data) {
      const title = typeof data.title === "string" ? data.title : "[restored]";
      const id = data.id as string | undefined;

      if (!id) {
        logger.warn({ data }, "Missing id for node rollback");
        return;
      }

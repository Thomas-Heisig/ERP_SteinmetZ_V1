...
export class IdempotencyStore {
  ...
  async isDuplicate(requestId: string, operationHash: string): Promise<boolean> {
  ...
  async markCompleted(requestId: string, operationHash: string, result: any, ttlSeconds = 60 * 60 * 24 * 365 * 10 /* 10 Jahre default */) {
    const client = await this.pool.connect();
    try {
      await client.query(
        ...
        [requestId, operationHash, result, new Date().toISOString(), ttlSeconds]
      );
    } finally {
      client.release();
    }

...
export class PostgresTransactionCoordinator implements TransactionCoordinator {
  ...
  async compensate(context: SagaContext): Promise<void> {
    ...
    try {
      // mark compensating
      await client.query(
        `UPDATE ${this.table} SET state = $1, updated_at = $2 WHERE saga_id = $3`,
        ["COMPENSATING", new Date().toISOString(), context.sagaId],
      );
  ...
  async getPendingSagas(): Promise<SagaContext[]> {
    ...
    try {
      const res = await client.query(
        `SELECT saga_id, process_id, state, data, current_step, created_at FROM ${this.table} WHERE state IN ('PENDING','COMPENSATING','FAILED')`,
      );
